...
class PageSerializer(object):
    def __init__(self, pagination_obj, **kwargs):
        ...
        self.data = {}
        self.serialize_items = kwargs.get('serialize_items', True)
        if self.serialize_items:
            self.items = [self.get_dto(resource, **kwargs) for resource in pagination_obj.items]
...
def get_success_response(messages, data=None, status_code=200):
    if type(messages) == list:
        msgs = messages
    elif type(messages) == str or type(messages) == unicode:
        msgs = [messages]
    else:
        msgs = []
